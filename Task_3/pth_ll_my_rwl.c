...
/* --- My rwlock implementation (writer-preference) --- */
typedef struct {
    pthread_mutex_t mutex;
    pthread_cond_t  can_read;
    pthread_cond_t  can_write;
    int readers;          /* number of active readers */
    int writers;          /* 1 if a writer holds the lock, 0 otherwise */
    int write_requests;   /* number of writers waiting */
} my_rwlock_t;
...
void my_rwlock_rdlock(my_rwlock_t *rw) {
    ...
    /* Wait while there is an active writer OR there are waiting writers (writer-preference) */
...
void my_rwlock_unlock(my_rwlock_t *rw) {
    ...
    if (rw->writers) {
        ...
        /* reader releasing */
        rw->readers--;
        if (rw->readers == 0 && rw->write_requests > 0) {
            /* wake one writer */
            pthread_cond_signal(&rw->can_write);
        }
    }
...
/* --- End of my rwlock implementation --- */
...
void* Thread_work(void* rank) {
   ...
   for (i = 0; i < ops_per_thread; i++) {
      ...
      if (which_op < search_percent) {
         my_rwlock_rdlock(&myrwlock);
         Member(val);
         my_rwlock_unlock(&myrwlock);
         my_member_count++;
      } else if (which_op < search_percent + insert_percent) {
         my_rwlock_wrlock(&myrwlock);
         Insert(val);
         my_rwlock_unlock(&myrwlock);
         my_insert_count++;
      } else { /* delete */
